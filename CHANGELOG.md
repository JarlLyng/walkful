...
### Changed
- **Design tokens updated** from iamjarl-design 1.1.0 to 1.6.0. Additive, so nothing in the app changes visually (#131).
...
- **Walkful Pro** — a one-time (non-consumable) StoreKit 2 unlock for Insights and the interval-walking coach. Calm paywall, restore purchases, on-device entitlement check (no subscription, no server). Today stays free; the gated parts (Insights, interval coach) show a paywall. Includes a local `Walkful.storekit` config for testing (#5).
